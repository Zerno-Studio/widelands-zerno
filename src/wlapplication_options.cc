...
// Build complex shortcut records in loops. A large initializer_list of
// strings, sets and maps expands into enormous startup functions with Asyncify.
#ifdef __EMSCRIPTEN__
__attribute__((noinline))
#endif
static std::vector<KeyboardShortcutInfo> make_fastplace_defaults(
   std::initializer_list<std::pair<const char*, const char*>> defaults) {
 std::vector<KeyboardShortcutInfo> result;
 result.reserve(defaults.size());
 for (const auto& entry : defaults) {
  result.emplace_back(std::set<KeyboardShortcutScope>{KeyboardShortcutScope::kGame},
                      keysym(SDLK_UNKNOWN), kFastplaceGroupPrefix + entry.first, entry.second);
 }
 return result;
}

static const std::vector<KeyboardShortcutInfo> kFastplaceDefaults = make_fastplace_defaults({
#define FP(name, descname) {name, descname}
...
// The scope initializer_lists only live through the make_shortcuts call;
// every scope, name and description is copied into the owning map there.
struct ShortcutDefaultData {
 std::initializer_list<KeyboardShortcutScope> scopes;
 SDL_Keysym key;
 const char* name;
 const char* description;
 constexpr ShortcutDefaultData(std::initializer_list<KeyboardShortcutScope> s,
                               SDL_Keysym k, const char* n, const char* d)
    : scopes(s), key(k), name(n), description(d) {}
};

#ifdef __EMSCRIPTEN__
__attribute__((noinline))
#endif
static std::map<KeyboardShortcut, KeyboardShortcutInfo> make_shortcuts(
   std::initializer_list<std::pair<KeyboardShortcut, ShortcutDefaultData>> defaults) {
 std::map<KeyboardShortcut, KeyboardShortcutInfo> result;
 for (const auto& entry : defaults) {
  const auto& d = entry.second;
  result.emplace(entry.first, KeyboardShortcutInfo(std::set<KeyboardShortcutScope>(d.scopes),
                                                  d.key, d.name, d.description));
 }
 return result;
}

static std::map<KeyboardShortcut, KeyboardShortcutInfo> shortcuts_ = make_shortcuts({
   {KeyboardShortcut::kMainMenuNew, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                         keysym(SDLK_n),
                                                         "mainmenu_new",
                                                         gettext_noop("New Game"))},
   {KeyboardShortcut::kMainMenuLoad, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                          keysym(SDLK_l),
                                                          "mainmenu_load",
                                                          gettext_noop("Load Game"))},
   {KeyboardShortcut::kMainMenuReplay, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                            keysym(SDLK_r),
                                                            "mainmenu_replay",
                                                            gettext_noop("Watch Replay"))},
   {KeyboardShortcut::kMainMenuLoadReplay, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                                keysym(SDLK_g),
                                                                "mainmenu_load_replay",
                                                                gettext_noop("Load Replay"))},
   {KeyboardShortcut::kMainMenuReplayLast,
    ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                         keysym(SDLK_d),
                         "mainmenu_replay_last",
                         gettext_noop("Watch Latest Replay"))},
   {KeyboardShortcut::kMainMenuRandomMatch, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                                 keysym(SDLK_z),
                                                                 "mainmenu_random",
                                                                 gettext_noop("New Random Game"))},
   {KeyboardShortcut::kMainMenuTutorial, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                              keysym(SDLK_t),
                                                              "mainmenu_tutorial",
                                                              gettext_noop("Tutorials"))},
   {KeyboardShortcut::kMainMenuCampaign, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                              keysym(SDLK_h),
                                                              "mainmenu_campaign",
                                                              gettext_noop("Campaigns"))},
   {KeyboardShortcut::kMainMenuSP, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                        keysym(SDLK_s),
                                                        "mainmenu_sp",
                                                        gettext_noop("Singleplayer"))},
   {KeyboardShortcut::kMainMenuMP, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                        keysym(SDLK_m),
                                                        "mainmenu_mp",
   ...
   {KeyboardShortcut::kMainMenuEditorLoad, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                                keysym(SDLK_b),
                                                                "mainmenu_editor_load",
                                                                gettext_noop("Editor – Load Map"))},
   {KeyboardShortcut::kMainMenuEditorNew, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                               keysym(SDLK_k),
                                                               "mainmenu_editor_new",
   ...
   {KeyboardShortcut::kMainMenuQuit, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                          keysym(SDLK_ESCAPE),
                                                          "mainmenu_quit",
                                                          gettext_noop("Exit Widelands"))},
   {KeyboardShortcut::kMainMenuAbout, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                           keysym(SDLK_F1),
                                                           "mainmenu_about",
                                                           gettext_noop("About"))},
   {KeyboardShortcut::kMainMenuAddons, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                            keysym(SDLK_a),
                                                            "mainmenu_addons",
                                                            gettext_noop("Add-Ons"))},
   {KeyboardShortcut::kMainMenuLAN, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                         keysym(SDLK_p),
                                                         "mainmenu_lan",
                                                         gettext_noop("LAN / Direct IP"))},
   {KeyboardShortcut::kMainMenuLobby, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                           keysym(SDLK_j),
                                                           "mainmenu_lobby",
                                                           gettext_noop("Metaserver Lobby"))},
   {KeyboardShortcut::kMainMenuLogin, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                           keysym(SDLK_u),
                                                           "mainmenu_login",
                                                           gettext_noop("Internet Login"))},
   {KeyboardShortcut::kMainMenuOptions, ShortcutDefaultData({KeyboardShortcutScope::kMainMenu},
                                                             keysym(SDLK_o),
                                                             "mainmenu_options",
                                                             gettext_noop("Options"))},

   {KeyboardShortcut::kCommonBuildhelp,
    ShortcutDefaultData({KeyboardShortcutScope::kGame, KeyboardShortcutScope::kEditor},
                         keysym(SDLK_SPACE),
                         "buildhelp",
   ...
   {KeyboardShortcut::kCommonContextMenu, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                               keysym(SDLK_MENU),
                                                               "context_menu",
                                                               gettext_noop("Context Menu"))},
   {KeyboardShortcut::kCommonTextCut, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                           keysym(SDLK_x, kDefaultCtrlModifier),
                                                           "cut",
                                                           gettext_noop("Cut Text"))},
   {KeyboardShortcut::kCommonTextCopy, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                            keysym(SDLK_c, kDefaultCtrlModifier),
                                                            "copy",
                                                            gettext_noop("Copy Text"))},
   {KeyboardShortcut::kCommonTextPaste, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                             keysym(SDLK_v, kDefaultCtrlModifier),
                                                             "paste",
                                                             gettext_noop("Paste Text"))},
   {KeyboardShortcut::kCommonSelectAll, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                             keysym(SDLK_a, kDefaultCtrlModifier),
                                                             "selectall",
                                                             gettext_noop("Select All"))},
   {KeyboardShortcut::kCommonDeleteItem, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                              keysym(SDLK_DELETE),
                                                              "delete",
                                                              gettext_noop("Delete Item"))},
   {KeyboardShortcut::kCommonTooltipAccessibilityMode,
    ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                         keysym(SDLK_F2),
                         "tt_access_mode",
                         gettext_noop("Tooltip Accessibility Mode Key"))},
   {KeyboardShortcut::kCommonFullscreen, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                              keysym(SDLK_f, kDefaultCtrlModifier),
                                                              "fullscreen",
                                                              gettext_noop("Toggle Fullscreen"))},
   {KeyboardShortcut::kCommonScreenshot, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                              keysym(SDLK_F11),
                                                              "screenshot",
                                                              gettext_noop("Take Screenshot"))},
   {KeyboardShortcut::kCommonChangeMusic, ShortcutDefaultData({KeyboardShortcutScope::kGlobal},
                                                               keysym(SDLK_F9),
                                                               "change_music",
   ...
   {KeyboardShortcut::kEditorNewMap, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                          keysym(SDLK_n, kDefaultCtrlModifier),
                                                          "editor_new_map",
                                                          gettext_noop("New Map"))},
   {KeyboardShortcut::kEditorNewRandomMap,
    ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                         keysym(SDLK_r, kDefaultCtrlModifier),
                         "editor_new_random_map",
                         gettext_noop("New Random Map"))},
   {KeyboardShortcut::kEditorUploadMap, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                             keysym(SDLK_u, kDefaultCtrlModifier),
                                                             "editor_upload_map",
                                                             gettext_noop("Publish Map Online"))},
   {KeyboardShortcut::kEditorMapOptions, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                              keysym(SDLK_o, kDefaultCtrlModifier),
                                                              "editor_mapoptions",
                                                              gettext_noop("Map Options"))},
   {KeyboardShortcut::kEditorUndo, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                        keysym(SDLK_z, kDefaultCtrlModifier),
                                                        "editor_undo",
                                                        gettext_noop("Undo"))},
   {KeyboardShortcut::kEditorRedo, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                        keysym(SDLK_y, kDefaultCtrlModifier),
                                                        "editor_redo",
                                                        gettext_noop("Redo"))},
   {KeyboardShortcut::kEditorPicker, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                          keysym(SDLK_k),
                                                          "editor_picker",
   ...
   {KeyboardShortcut::kEditorTerrain, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                           keysym(SDLK_t, KMOD_SHIFT),
                                                           "editor_terrain",
                                                           gettext_noop("Terrain Tool"))},
   {KeyboardShortcut::kEditorImmovables, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                              keysym(SDLK_o, KMOD_SHIFT),
                                                              "editor_immovables",
                                                              gettext_noop("Immovables Tool"))},
   {KeyboardShortcut::kEditorAnimals, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                           keysym(SDLK_a, KMOD_SHIFT),
                                                           "editor_animals",
                                                           gettext_noop("Animals Tool"))},
   {KeyboardShortcut::kEditorResources, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                             keysym(SDLK_r, KMOD_SHIFT),
                                                             "editor_resources",
                                                             gettext_noop("Resources Tool"))},
   {KeyboardShortcut::kEditorPortSpaces, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                              keysym(SDLK_e, KMOD_SHIFT),
                                                              "editor_port_spaces",
                                                              gettext_noop("Port Spaces Tool"))},
   {KeyboardShortcut::kEditorInfo, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                        keysym(SDLK_i, KMOD_SHIFT),
                                                        "editor_info",
                                                        gettext_noop("Info Tool"))},
   {KeyboardShortcut::kEditorMapOrigin, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                             keysym(SDLK_x, KMOD_SHIFT),
                                                             "editor_map_origin",
                                                             gettext_noop("Map Origin Tool"))},
   {KeyboardShortcut::kEditorMapSize, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                           keysym(SDLK_z, KMOD_SHIFT),
                                                           "editor_map_size",
                                                           gettext_noop("Map Size Tool"))},
   {KeyboardShortcut::kEditorPlayers, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                           keysym(SDLK_p, KMOD_SHIFT),
                                                           "editor_players",
                                                           gettext_noop("Players Menu"))},
   {KeyboardShortcut::kEditorToolHistory, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                               keysym(SDLK_h),
                                                               "editor_tool_history",
   ...
   {KeyboardShortcut::kEditorShowhideGrid, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                                keysym(SDLK_g),
                                                                "editor_showhide_grid",
                                                                gettext_noop("Toggle Grid"))},
   {KeyboardShortcut::kEditorShowhideOceans, ShortcutDefaultData({KeyboardShortcutScope::kEditor},
                                                                  keysym(SDLK_z),
...
#define EDITOR_TOOLSIZE(radius, key)                                                               \
	{                                                                                               \
		KeyboardShortcut::kEditorToolsize##radius,                                                   \
		   ShortcutDefaultData({KeyboardShortcutScope::kEditor}, keysym(SDLK_##key),                \
...
#define EDITOR_TOOLGAP(radius, key)                                                                \
	{                                                                                               \
		KeyboardShortcut::kEditorToolgap##radius,                                                    \
		   ShortcutDefaultData({KeyboardShortcutScope::kEditor}, keysym(SDLK_##key, KMOD_SHIFT),    \
...
#undef EDITOR_TOOLGAP
   ...
   {KeyboardShortcut::kInGameSoundOptions,
    ShortcutDefaultData({KeyboardShortcutScope::kGame},
                         keysym(SDLK_o, kDefaultCtrlModifier),
                         "game_sound_options",
   ...
   {KeyboardShortcut::kInGameShowhideCensus, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                                  keysym(SDLK_c),
   ...
   {KeyboardShortcut::kInGameStatsWares, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                              keysym(SDLK_p),
                                                              "game_stats_wares",
   ...
   {KeyboardShortcut::kInGameStatsStock, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                              keysym(SDLK_i),
                                                              "game_stats_stock",
   ...
   {KeyboardShortcut::kInGameObjectives, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                              keysym(SDLK_t),
                                                              "game_objectives",
                                                              gettext_noop("Objectives"))},
   {KeyboardShortcut::kInGameDiplomacy, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                             keysym(SDLK_d),
                                                             "game_diplomacy",
   ...
   {KeyboardShortcut::kInGameSpeedReset, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                              keysym(SDLK_PAUSE, KMOD_SHIFT),
                                                              "game_speed_reset",
                                                              gettext_noop("Reset Game Speed"))},
   {KeyboardShortcut::kInGamePause, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                         keysym(SDLK_PAUSE),
                                                         "game_pause",
   ...
   {KeyboardShortcut::kInGamePinnedNote, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                              keysym(SDLK_F8),
                                                              "game_pinned_note",
                                                              gettext_noop("Pinned Note"))},
   {KeyboardShortcut::kInGameMessagesGoto, ShortcutDefaultData({KeyboardShortcutScope::kGame},
                                                                keysym(SDLK_g),
                                                                "game_msg_goto",
...
#define QUICKNAV(i)                                                                                \
	{KeyboardShortcut::kInGameQuicknavSet##i,                                                       \
	 ShortcutDefaultData({KeyboardShortcutScope::kGame}, keysym(SDLK_##i, kDefaultCtrlModifier),   \
	                      "game_quicknav_set_" #i, gettext_noop("Set Landmark #%d"))},              \
	{                                                                                               \
		KeyboardShortcut::kInGameQuicknavGoto##i,                                                    \
		   ShortcutDefaultData({KeyboardShortcutScope::kGame}, keysym(SDLK_##i),                    \
...
#ifdef __EMSCRIPTEN__
 if (!rt_escape) return ""; // Desktop menu accelerator column is redundant on touch.
 const std::string label = "Actions → " + to_string(id);
 return rt_escape ? richtext_escape(label) : label;
#endif
...
void mobile_publish_shortcuts() {
 EM_ASM({ Module['mobileActions'] = []; }, 0);
 for (const auto& pair : shortcuts_) {
  if (!is_real(pair.first)) continue;
  const auto& info = pair.second;
  unsigned scopes = 0;
  for (auto scope : info.scopes) scopes |= 1u << static_cast<unsigned>(scope);
  const std::string label = to_string(pair.first);
  EM_ASM({ Module['mobileActions'].push({id:$0,name:UTF8ToString($1),label:UTF8ToString($2),scopes:$3,bound:!!$4}); },
         static_cast<int>(pair.first), info.internal_name.c_str(), label.c_str(), scopes,
         info.current_shortcut.sym != SDLK_UNKNOWN);
 }
}
#endif
